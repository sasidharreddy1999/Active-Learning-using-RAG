        ...
        for i in range(54):
            ...
            if os.path.exists(file_path):
                with open(file_path, 'r', encoding='utf-8') as file:
                    ...
                    anchoring_correctness = acc(anchoring_output,answer_key)

                    associate_output = data["associate"][-1]["content"]
                    associate_correctness = acc(associate_output, answer_key)

                    logician_output = data["logician"][-1]["content"]
                    logician_correctness = acc(logician_output, answer_key)

                    cognition_output = data["cognition"][-1]["content"]
                    cognition_correctness = acc(cognition_output, answer_key)
